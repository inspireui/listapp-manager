...
function add_events_to_json_api()
{
    ...
    if (isset($wp_taxonomies['job_listing'])) {
        //be sure to set this to the name of your taxonomy!
        $taxonomy_name = array('job_listing_category', 'job_listing_region');
...
/**
 * register add more field to rest api
 */
...
function get_comments_ratings($object, $field_name, $request)
{
    ...
    foreach ($comments as $comment) {
        $current_rating = get_comment_meta($comment->comment_ID, 'rating', true);
...
{
    $term_meta_id = get_term_meta($object['id'], 'thumbnail_id', true);
...
{
    $id = $request['id'];
    $countRating = get_comment_meta($id, 'rating', true);
    return new WP_REST_Response($countRating, 200);
}

/**
 * Get cost for boooking
 * @param $object
 * @param $field_name
 * @param $request
 * @return array
 ...
 * @return mixed
 */
function get_job_hours($object, $field_name, $request)
{
